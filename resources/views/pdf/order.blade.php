...
<div class="">
    ...
    <main>
        ...
        <br>
        <h5>Afleveradres</h5>
        <p>Adres: {{$user["address"] . " " . $user["house_number"]}}</p>
        ...
        <table class="table table-striped">
            ...
            @foreach($products as $order)
                <tr>
                    ...
                    <td style="text-align: end">&euro;{{$order->option_price}}</td>
                    <td style="text-align: end">&euro;{{$order->amount * $order->option_price}}</td>
                </tr>
            @endforeach
            </tbody>
        </table>
    ...
    <footer>
        <div class="totaal border border-dark p-2" style="text-align: right">
            <p>Totaal exlusief BTW: &euro;{{number_format($totalPrice / 1.21, 2)}}</p>
            <p>Verzendkosten: &euro;4,15</p>
            <p>21,00% BTW: &euro;{{number_format(($totalPrice / 121) * 21, 2)}}</p>
            <br>
            <p>Totaal inclusief BTW: &euro;{{$totalPrice}}</p>
        </div>
    </footer>
</div>
</body>
